...
%.a: | %.verilator_deplist.tmp
	$(ECHO) " AR       $(notdir $@)"
	$(Q)$(foreach L, $(filter-out %.a,$^), $(shell echo $L >>$@.verilator_deplist.tmp))
	$(Q)if test $(words $(filter %.a,$^)) -eq 0; then \
		$(RM) -f $@; \
	...
	; $(RM) -rf $@.verilator_deplist.tmp $@.tmpdir $(REDIRECT)
